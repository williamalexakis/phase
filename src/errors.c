...
static ErrorLocation normalize_location(ErrorLocation loc) {
    ...
    if (!loc.file)
        loc.file = g_error_file;
    if (loc.col_end < loc.col_start && loc.col_end != 0)
...
static char *trim_expected_token(const char *expected) {
    ...
    while (len > 0 && (expected[len - 1] == ' ' || expected[len - 1] == '\t'))
        len--;

    if (len == 0)
        return NULL;

    if (len >= 2 && ((expected[0] == '\'' && expected[len - 1] == '\'') ||
...
suggest_remove_span(const char *line_text, ErrorLocation loc, va_list args) {
    ...
    if (start > line_len)
        start = line_len;
    if (end >= line_len)
        end = line_len ? line_len - 1 : 0;
    if (end < start)
        end = start;

    size_t remove_len = (end >= start) ? (end - start + 1) : 0;
    if (remove_len > line_len)
        remove_len = line_len;
...
static const char *placeholder_for_expected(const char *expected) {
    ...
    if (strcmp(expected, "int") == 0)
        return "0";
    if (strcmp(expected, "float") == 0)
        return "0.0";
    if (strcmp(expected, "str") == 0 || strcmp(expected, "string") == 0)
        return "\"\"";
    if (strcmp(expected, "bool") == 0 || strcmp(expected, "boolean") == 0)
        return "false";
...
static char *suggest_type_mismatch_fix(const char   *line_text,
    ...
    if (matched_len > 0) {
        ...
        memcpy(out + prefix_len + actual_len,
               type_start + matched_len,
               suffix_len);
...
bool unicode_available(void) {
    ...
    if (term) {
        ...
        if (strcmp(term, "dumb") == 0)
            return false;
        if (strstr(term, "vt100") != NULL)
            return false;
        if (strstr(term, "ansi") != NULL)
            return false;
    }
...
static noreturn void error_emit(ErrorLocation loc, ErrorType code, ...) {
    ...
    if (!info) {
        ...
        fprintf(stderr,
                "%s%s Fatal Error [%d]:%s Unknown error.\n",
                FG_RED_BOLD,
                bar_main,
                code,
                RESET);
        fprintf(stderr,
                "%s%s Help:%s Unavailable (INTERNAL ERROR).\n",
                FG_PURPLE_BOLD,
                bar_sub,
                RESET);
        exit_phase(1);
...
// Internal errors
void error_oom(void) {
    error_emit((ErrorLocation){0}, ERR_OOM);
}
void error_complexity(void) {
    error_emit((ErrorLocation){0}, ERR_COMPLEXITY);
}
...
void error_expect_expression(ErrorLocation loc) {
    error_emit(loc, ERR_EXPECT_EXPRESSION);
}
void error_expect_statement(ErrorLocation loc) {
    error_emit(loc, ERR_EXPECT_STATEMENT);
}
void error_invalid_token(ErrorLocation loc) {
    error_emit(loc, ERR_INVALID_TOK);
}
void error_multiple_entry(ErrorLocation loc) {
    error_emit(loc, ERR_MANY_ENTRY);
}
...
void error_invalid_opcode(ErrorLocation loc, int op) {
    error_emit(loc, ERR_INVALID_OPCODE, op);
}
void error_invalid_var_index(ErrorLocation loc, size_t var_count) {
    error_emit(loc, ERR_INVALID_VAR_INDEX, var_count);
}
void error_invalid_const_index(ErrorLocation loc, size_t const_count) {
    error_emit(loc, ERR_INVALID_CONST_INDEX, const_count);
}
void error_vm_oob(ErrorLocation loc) {
    error_emit(loc, ERR_VM_POS_OOB);
}
void error_wrong_var_init(ErrorLocation loc,
                          size_t        var_count,
                          size_t        init_count) {
